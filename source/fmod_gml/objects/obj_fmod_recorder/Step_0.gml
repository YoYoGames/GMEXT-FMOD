...
if ((channel == -1) && (samples_recorded >= adjusted_latency))
{
    ...
    var _channel = fmod_system_play_sound(sound_index, fmod_system_get_master_channel_group(), true);

	if (_channel != 0)
	{
		channel = _channel;

		// Seek playback to record position to avoid playing ahead
		fmod_channel_set_position(channel, (_record_pos % sound_length), FmodTimeUnit.Pcm);
		fmod_channel_control_set_paused(channel, false);
	}
}
